...
const timeIn = async (req, res) => {
  try {
    ...
    const activeLog = await prisma.timeLogs.findFirst({ where: { userId, status: true } });
    ...
    if (!isWithinAllowedLocation) {
      return res.status(400).json({ message: "Punch location is outside allowed areas." });
    }
    ...
    await prisma.users.update({ where: { id: userId }, data: { status: true } });
...
const timeOut = async (req, res) => {
  try {
    ...
    await prisma.users.update({ where: { id: userId }, data: { status: false } });
...
const getMonthlyLogs = async (req, res) => {
  try {
    ...
    const logs = await prisma.timeLogs.findMany({
      where: { userId: Number(userId), timeInAt: { gte: firstDate, lte: lastDate } },
      orderBy: { createdAt: "asc" },
    });
    return res.status(200).json({ message: "Monthly logs retrieved successfully.", data: logs });
  } catch (error) {
...
const getRangeLogs = async (req, res) => {
  try {
    ...
    const requestingUser = await prisma.users.findUnique({ where: { id: req.user.id } });
    ...
    if (requestingUser.role !== "admin" && requestingUser.role !== "superadmin") {
      ...
      if (!userId) userId = req.user.id;
        ...
        const targetUser = await prisma.users.findUnique({ where: { id: Number(userId) } });
    ...
    return res.status(200).json({ message: "Range logs retrieved successfully.", data: logs });
  } catch (error) {
...
const getUserTimeLog = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: "Latest time log retrieved successfully.", data: timeLog });
  } catch (error) {
...
const coffeeBreakToggle = async (req, res) => {
  try {
    ...
    return res.status(400).json({ message: "Maximum coffee breaks reached for this shift." });
  } catch (error) {
    console.error("Error in coffeeBreakToggle:", error);
    return res.status(500).json({ message: "Internal server error while toggling coffee break." });
  }
};
...
const lunchBreakToggle = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: "Internal server error while toggling lunch break." });
  }
};
