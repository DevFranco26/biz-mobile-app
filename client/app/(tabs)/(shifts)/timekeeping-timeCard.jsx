// File: app/(tabs)/(shifts)/timekeeping-timecard.jsx
...
import { combineDateTime, formatDateWithDay, formatTime, formatTotalDuration } from '../../../utils/timeUtils'
...
const TimeCard = () => {
  ...
  const fetchTimeLogs = async () => {
    ...
    try {
      ...
      if (!token) {
        Alert.alert('Authentication Error', 'You are not logged in. Please sign in again.', [{ text: 'OK' }])
        setLoading(false)
        setRefreshing(false)
        return
      }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: isLightTheme ? '#FFFFFF' : '#0f172a', paddingTop: insets.top + 60 }}>
      <View style={{ paddingHorizontal: 16, paddingTop: 10 }}>
        ...
        {rangeType === 'Custom' && (
          <View className="mb-6">
            <Pressable
              ...
              className={`p-4 rounded-lg mb-3 ${isLightTheme ? 'bg-slate-200' : 'bg-slate-800'}`}
            >
            ...
            <Pressable
              ...
              className={`p-4 rounded-lg ${isLightTheme ? 'bg-slate-200' : 'bg-slate-800'}`}
            >
              <Text className={`${isLightTheme ? 'text-slate-700' : 'text-slate-400'}`}>
        ...
        {rangeType !== 'Custom' && (
          <View className="flex-row items-center my-2">
            ...
            <View className="flex-row items-center ml-auto gap-3">
              <Pressable onPress={handlePrev} className={`p-2 rounded-full ${isLightTheme ? 'bg-white' : 'bg-slate-900'}`}>
                <FontAwesome5 name="arrow-left" size={20} color={isLightTheme ? '#334155' : '#cbd5e1'} />
              </Pressable>
              <Pressable onPress={handleNext} className={`p-2 rounded-full ${isLightTheme ? 'bg-white' : 'bg-slate-900'}`}>
                <FontAwesome5 name="arrow-right" size={20} color={isLightTheme ? '#334155' : '#cbd5e1'} />
              </Pressable>
            </View>
          </View>
        )}
        ...
        <View className={`p-4 rounded-lg mb-4 flex-row justify-between items-center ${isLightTheme ? 'bg-slate-100' : 'bg-slate-800'}`}>
          <Text className={`text-lg font-medium ${isLightTheme ? 'text-slate-700' : 'text-slate-300'}`}>Total Hours:</Text>
          <Text className={`text-xl font-semibold ${isLightTheme ? 'text-slate-700' : 'text-slate-300'}`}>
        ...
        {loading ? (
          ...
          logsWithDuration.map((log, index) => {
            ...
            return (
              <View
                key={index}
                className={`px-4 py-2 rounded-lg mb-2 ${isLightTheme ? 'bg-slate-100' : 'bg-slate-800'}`}
              >
                <Text className={`text-lg font-medium ${isLightTheme ? 'text-slate-700' : 'text-slate-300'}`}>
                  {localDateFormatted}
                </Text>
                <Text className={`text-md ${isLightTheme ? 'text-slate-600' : 'text-slate-400'}`}>
                  {timeInFormatted} - {timeOutFormatted}
                </Text>
                <Text className={`text-sm text-right ${isLightTheme ? 'text-slate-600' : 'text-slate-400'}`}>
                  {log.duration}
                </Text>
              </View>
            )
