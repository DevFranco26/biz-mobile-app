...
import { View, Text, FlatList, Pressable, ActivityIndicator, Modal, TouchableOpacity, Alert, RefreshControl } from "react-native";
  ...
  const renderLeaveItem = ({ item }) => (
    <View className={`p-4 rounded-lg mb-4 ${isLightTheme ? "bg-slate-100" : "bg-slate-800"}`}>
      <View className="flex-row justify-between items-center mb-2">
        <Text className={`text-lg font-semibold ${isLightTheme ? "text-slate-800" : "text-slate-100"}`}>{item.type}</Text>
        <Text className={`font-semibold ${getStatusColor(item.status)}`}>{item.status}</Text>
      </View>

      {item.userId && (
        <View className="mt-1 flex-row items-start">
          <FontAwesome5 name="user-alt" size={18} color={isLightTheme ? "#374151" : "#9ca3af"} className="mr-1 mt-1" />
          <Text className={`text-sm my-auto ${isLightTheme ? "text-slate-700" : "text-slate-300"} flex-1`}>
            <Text className="font-bold">Requester:</Text> {requesterEmails[item.userId] || "Fetching requester email..."}
          </Text>
        </View>
      )}
      ...
      {item.approverId && (
        <View className="mt-1 flex-row items-start">
          <FontAwesome5 name="user-cog" size={18} color={isLightTheme ? "#374151" : "#9ca3af"} className="mr-1 mt-1" />
          <Text className={`text-sm my-auto ${isLightTheme ? "text-slate-700" : "text-slate-300"} flex-1`}>
            <Text className="font-bold">Approver:</Text> {approverEmails[item.approverId] || "Fetching approver email..."}
          </Text>
        </View>
      )}

      <View className="flex-row items-center mt-2">
        <FontAwesome5 name="arrow-alt-circle-up" size={20} color={isLightTheme ? "#374151" : "#9ca3af"} className="mr-2" />
      ...
      <View className="flex-row items-center mt-2">
        <FontAwesome5 name="arrow-alt-circle-down" size={20} color={isLightTheme ? "#374151" : "#9ca3af"} className="mr-2" />
        <Text className={`text-sm font-medium ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>
      ...
      {item.reason && (
        <View className="mt-1 flex-row items-start">
          <FontAwesome5 name="info-circle" size={20} color={isLightTheme ? "#374151" : "#9ca3af"} className="mr-2 mt-1 my-auto" />
          <Text className={`text-sm my-auto ${isLightTheme ? "text-slate-700" : "text-slate-300"} flex-1`}>
      ...
      {item.status === "Rejected" && item.rejectionReason && (
        <View className="mt-1 flex-row items-start">
          <AntDesign name="closecircle" size={20} color="#ef4444" className="mr-2 mt-1" />
          <Text className={`text-sm my-auto ${isLightTheme ? "text-slate-700" : "text-slate-300"} flex-1`}>
            <Text className="font-bold text-red-500">Rejected: </Text>
  ...
  return (
    <SafeAreaView className={`flex-1 px-4 ${isLightTheme ? "bg-white" : "bg-slate-900"}`} style={{ paddingTop: 60 }}>
        ...
        <View className="flex-row flex-wrap">
          {filterStatus !== "ALL" && (
            <View className={`flex-row items-center px-2 py-1 rounded-full mr-2 mb-1 ${isLightTheme ? "bg-slate-100" : "bg-slate-700"}`}>
              <Text className={`text-sm mr-1 ${isLightTheme ? "text-slate-900" : "text-slate-300"}`}>{filterStatus}</Text>
          ...
          {filterType !== "ALL" && (
            <View className={`flex-row items-center px-2 py-1 rounded-full mr-2 mb-1 ${isLightTheme ? "bg-slate-100" : "bg-slate-700"}`}>
              <Text className={`text-sm mr-1 ${isLightTheme ? "text-slate-900" : "text-slate-300"}`}>{filterType}</Text>
      ...
      >
        ...
        >
          <View className={`w-11/12 max-h-3/4 p-6 rounded-2xl ${isLightTheme ? "bg-slate-100" : "bg-slate-800"}`}>
            <View className="flex-row justify-between items-center mb-4">
            ...
            {!selectedFilterOption && (
              <View>
                ...
                >
                  <Ionicons name={getStatusIcon("Pending")} size={24} color={isLightTheme ? "#374151" : "#9ca3af"} className="mr-3" />
                ...
                >
                  <Ionicons name={getStatusIcon("All")} size={24} color={isLightTheme ? "#374151" : "#9ca3af"} className="mr-3" />
                  <Text className={`text-lg ${isLightTheme ? "text-slate-800" : "text-slate-100"}`}>Filter by Leave Type</Text>
                </Pressable>
              </View>
            )}
            ...
            {selectedFilterOption === "status" && (
              <View>
                {["Pending", "Approved", "Rejected"].map((status) => (
                  <Pressable
                    key={status}
                    className={`flex-row items-center mb-3 p-2 rounded-lg ${filterStatus === status ? "bg-slate-200" : ""}`}
                  ...
                  >
                    <Ionicons
                      ...
                      size={24}
                      color={filterStatus === status ? "#1e3a8a" : isLightTheme ? "#374151" : "#9ca3af"}
                      className="mr-3"
                    />
                    <Text className={`text-lg ${filterStatus === status ? "text-slate-900" : isLightTheme ? "text-slate-800" : "text-slate-100"}`}>
                      {status}
                    </Text>
                  </Pressable>
                ))}
            ...
            {selectedFilterOption === "type" && (
              <View>
                {["Sick Leave", "Vacation Leave", "Emergency Leave", "Maternity/Paternity Leave", "Casual Leave"].map((type) => (
                  <Pressable
                    key={type}
                    className={`flex-row items-center mb-3 p-2 rounded-lg ${filterType === type ? (isLightTheme ? "bg-slate-200" : "bg-slate-600") : ""}`}
                  ...
                  >
                    <Ionicons
                      ...
                      size={24}
                      color={filterType === type ? (isLightTheme ? "#44403c" : "#d6d3d1") : isLightTheme ? "#44403c" : "#d6d3d1"}
                      className="mr-3"
                    />
                    <Text
                      className={`text-lg ${
                        filterType === type ? (isLightTheme ? "text-slate-700" : "text-slate-300") : isLightTheme ? "text-slate-700" : "text-slate-300"
                      }`}
                    >
                      {type}
                    </Text>
      ...
      {loadingUserLeaves && !refreshing ? (
        ...
        <Text className={`text-center mt-10 text-base ${isLightTheme ? "text-red-700" : "text-red-300"}`}>{errorUserLeaves}</Text>
      ) : (
        <FlatList
          ...
          contentContainerStyle={
            getFilteredAndSortedLeaves().length === 0 ? { flexGrow: 1, justifyContent: "center", alignItems: "center" } : { paddingBottom: 40 }
          }
          ListEmptyComponent={
            <Text className={`text-center mt-10 text-base ${isLightTheme ? "text-slate-800" : "text-white"}`}>You have not submitted any leave requests.</Text>
          }
          refreshControl={
