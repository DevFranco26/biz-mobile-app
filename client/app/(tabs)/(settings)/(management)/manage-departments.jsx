...
const Departments = () => {
  ...
  const loadUsersForDepartment = (department) => {
    const current = users.filter((user) => user.departmentId === department.id);
    if (department?.supervisor && !current.some((user) => user.id === department.supervisor.id)) {
      current.push(department.supervisor);
    }
    const available = users.filter((user) => user.departmentId !== department.id && user.role !== "superadmin");
    ...
    <View className={`p-4 mb-3 rounded-lg flex-row justify-between items-center ${isLightTheme ? "bg-slate-100" : "bg-slate-800"}`}>
      ...
      <View className="flex-row">
        <Pressable onPress={() => openAssignUserModal(item)} className="my-auto">
          <FontAwesome5 name="user-edit" size={20} color={isLightTheme ? "#1e293b" : "#cbd5e1"} />
        </Pressable>
  ...
  const renderAssignedUserItem = ({ item }) => (
    <View className="flex-row items-center mb-3">
      <Ionicons name="person-circle-outline" size={24} color={isLightTheme ? "#1e293b" : "#cbd5e1"} className="mr-2" />
      <View style={{ flex: 1 }}>
      ...
      <TouchableOpacity onPress={() => handleDeleteUserFromDepartment(selectedDepartment.id, item.id, `${item.firstName} ${item.lastName}`)} className="ml-2">
  ...
  const handleDeleteUserFromDepartment = (departmentId, userId, userName) => {
    Alert.alert("Confirm Removal", `Are you sure you want to remove ${userName} from this department?`, [
      { text: "Cancel", style: "cancel" },
      {
        text: "Remove",
        style: "destructive",
  ...
  return (
    <SafeAreaView className={`flex-1 ${isLightTheme ? "bg-white" : "bg-slate-900"}`} edges={["top"]}>
      <View className="flex-row items-center px-4 py-3">
        ...
        <Text className={`text-lg font-bold ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>Departments</Text>
      </View>
      <View className="flex-row justify-between items-center px-4 mb-4 z-50">
        <Text className={`text-2xl font-bold ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>Departments</Text>
        <Pressable onPress={handleAddDepartment} className={`p-2 rounded-full ${isLightTheme ? "bg-white" : "bg-slate-900"}`}>
          <Ionicons name="add" size={24} color={isLightTheme ? "#1e293b" : "#cbd5e1"} />
        </Pressable>
      </View>
      {loading ? (
        <ActivityIndicator size="large" color={isLightTheme ? "#475569" : "#f9fafb"} style={{ marginTop: 48 }} />
      ) : (
        <FlatList
          data={departments}
          ...
          ListEmptyComponent={<Text className={`text-center mt-12 text-lg ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>No departments found.</Text>}
        />
      )}
      <Modal visible={editDeptModalVisible} animationType="fade" transparent={true} onRequestClose={() => setEditDeptModalVisible(false)}>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View className={`flex-1 justify-center items-center ${isLightTheme ? "bg-slate-950/70" : "bg-slate-950/70"}`}>
            ...
            >
              <View className={`p-6 rounded-2xl w-full ${isLightTheme ? "bg-white" : "bg-slate-900"}`} style={{ maxHeight: "100%" }}>
                ...
                <View className="flex-row justify-end">
                  <TouchableOpacity onPress={() => setEditDeptModalVisible(false)} className="mr-4">
                    <Text className={`text-base font-semibold my-auto ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>Cancel</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={handleSaveDepartmentEdits} className="bg-orange-500 py-3 px-6 rounded-lg">
      ...
      <Modal visible={assignUserModalVisible} animationType="fade" transparent={true} onRequestClose={() => setAssignUserModalVisible(false)}>
        <TouchableWithoutFeedback onPress={() => setAssignUserModalVisible(false)}>
          <View className={`flex-1 justify-center items-center ${isLightTheme ? "bg-black/50" : "bg-black/50"}`}>
            <KeyboardAvoidingView
              behavior={Platform.OS === "ios" ? "padding" : "height"}
              className="w-11/12"
              keyboardVerticalOffset={Platform.OS === "ios" ? 60 : 20}
            >
              <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                <View className={`p-6 rounded-2xl w-full ${isLightTheme ? "bg-white" : "bg-slate-900"}`} style={{ maxHeight: "100%" }}>
                  ...
                  <Text className={`text-sm font-medium mb-2 ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>Current Users:</Text>
                  {currentUsers.length > 0 ? (
                    <FlatList
                      data={currentUsers}
                      keyExtractor={(user) => user.id.toString()}
                      renderItem={({ item }) => <Text className={`text-sm mb-1 ${isLightTheme ? "text-slate-700" : "text-slate-400"}`}>{item.email}</Text>}
                      style={{ maxHeight: 100, marginBottom: 10 }}
                      ListEmptyComponent={
                        <Text className={`text-sm mb-1 ${isLightTheme ? "text-slate-700" : "text-slate-400"}`}>No users assigned to this department.</Text>
                      }
                    />
                  ) : (
                    <Text className={`text-sm mb-1 ${isLightTheme ? "text-slate-700" : "text-slate-400"}`}>No users assigned to this department.</Text>
                  )}
                  <Text className={`text-sm font-medium mt-4 mb-2 ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>Add User:</Text>
                  <DropDownPicker
                    open={dropdownOpen}
                    ...
                    textStyle={{ color: isLightTheme ? "#374151" : "#E5E7EB" }}
                    style={{
                      borderWidth: 0,
                    ...
                    ListItemComponent={({ item }) => <Text className={`text-base ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>{item.label}</Text>}
                    Icon={() => <Ionicons name="chevron-down" size={20} color={isLightTheme ? "#374151" : "#E5E7EB"} />}
                  />
                  <View className="flex-row justify-end">
                    <TouchableOpacity onPress={() => setAssignUserModalVisible(false)} className="mr-4">
                      <Text className={`text-base font-semibold my-auto ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>Cancel</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={handleAddUserToDepartment} className="bg-orange-500 py-3 px-6 rounded-lg">
                      <Text className="text-white text-base font-semibold">Add User</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              </TouchableWithoutFeedback>
