// File: app/(tabs)/(settings)/(management)/payroll-generate-payroll.jsx
    ...
    return (
      <SafeAreaView className={`flex-1 ${isLightTheme ? "bg-white" : "bg-slate-900"}`} edges={["top"]}>
        <View className="flex-1 justify-center items-center">
          ...
          <Text className={`mt-4 ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>Loading Manual Payroll Calculation...</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView className={`flex-1 ${isLightTheme ? "bg-white" : "bg-slate-900"}`} edges={["top"]}>
      <View className="flex-row items-center px-4 py-4">
        ...
        <Text className={`text-xl font-bold ${isLightTheme ? "text-slate-800" : "text-white"}`}>Generate Payroll</Text>
      </View>
      <View className="flex-1 px-4 mt-4">
        <Text className={`font-semibold mb-2 ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>Select User</Text>
        <DropDownPicker
          open={calcUserOpen}
          value={calcUserValue}
          items={calcUserItems}
        ...
        />
        <Text className={`font-semibold mb-1 mt-4 ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>Start Date</Text>
        <Pressable className={`mb-4 p-4 rounded-lg ${isLightTheme ? "bg-slate-100" : "bg-slate-800"}`} onPress={() => setShowStartPicker(true)}>
          <Text className={isLightTheme ? "text-slate-800" : "text-slate-100"}>{formatDate(startDate) || "YYYY-MM-DD"}</Text>
        </Pressable>
        {showStartPicker && (
          <DateTimePicker
            value={startDate}
        ...
        <Text className={`font-semibold mb-1 ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>End Date</Text>
        <Pressable className={`mb-4 p-4 rounded-lg ${isLightTheme ? "bg-slate-100" : "bg-slate-800"}`} onPress={() => setShowEndPicker(true)}>
          <Text className={isLightTheme ? "text-slate-800" : "text-slate-100"}>{formatDate(endDate) || "YYYY-MM-DD"}</Text>
        </Pressable>
        {showEndPicker && (
          <DateTimePicker
            value={endDate}
        ...
        <Pressable className="bg-orange-500 p-4 rounded-lg" onPress={handleCalculatePayrollManually}>
          <Text className="text-white text-center font-semibold">Generate Payroll</Text>
        </Pressable>
      </View>
    </SafeAreaView>
  );
