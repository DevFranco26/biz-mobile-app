...
const PayrollRecords = () => {
  ...
  const [filterItems, setFilterItems] = useState([{ label: "All Users", value: "all" }]);
  ...
  const renderPayrollItem = ({ item }) => {
    ...
    return (
      <View className={`p-4 mb-3 rounded-lg flex-col ${isLightTheme ? "bg-slate-100" : "bg-slate-800"}`}>
        <Text className={`text-base font-semibold ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>{userName}</Text>
        <Text className={`text-sm mt-1 ${isLightTheme ? "text-slate-600" : "text-slate-300"}`}>
          Period: {format(new Date(item.startDate), "MMMM d, yyyy")} ~ {format(new Date(item.endDate), "MMMM d, yyyy")}
        </Text>
        <View className="flex-row items-center mt-1">
          <Ionicons name="cash-outline" size={16} color={getPayTypeColor(item.payType)} className="mr-1" />
        ...
        <Text className={`text-sm mt-1 ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>Gross Pay: ${grossPay}</Text>
        <Text className={`text-sm mt-1 ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>Net Pay: ${netPay}</Text>
      </View>
    );
  };
    ...
    return (
      <SafeAreaView className={`flex-1 ${isLightTheme ? "bg-white" : "bg-slate-900"}`} edges={["top"]}>
        <View className="flex-1 justify-center items-center">
          ...
          <Text className={`mt-4 ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>Loading Payroll Records...</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView className={`flex-1 ${isLightTheme ? "bg-white" : "bg-slate-900"}`} edges={["top"]}>
      <View className="flex-row items-center px-4 py-4">
        ...
        <Text className={`text-xl font-bold ${isLightTheme ? "text-slate-800" : "text-white"}`}>Payroll Records</Text>
      </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          !loading &&
          !usersLoading && <Text className={`text-center mt-12 text-lg ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>No payroll records found.</Text>
        }
      />
    </SafeAreaView>
  );
