// File: app/(tabs)/(settings)/(management)/payroll-payroll-settings.jsx
...
const PayrollSettings = () => {
  ...
  const { payrollSettings, fetchPayrollSettings, updatePayrollSettings, loading } = usePayrollStore();
    ...
    return (
      <SafeAreaView className={`flex-1 ${isLightTheme ? "bg-white" : "bg-slate-900"}`} edges={["top"]}>
        <View className="flex-1 justify-center items-center">
          ...
          <Text className={`mt-4 ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>Loading Payroll Settings...</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    return (
      <SafeAreaView className={`flex-1 ${isLightTheme ? "bg-white" : "bg-slate-900"}`} edges={["top"]}>
        <View className="flex-row items-center px-4 py-4">
          ...
          <Text className={`text-xl font-bold ${isLightTheme ? "text-slate-800" : "text-white"}`}>Company Payroll Settings</Text>
        </View>
        <View className="flex-1 px-4 mt-4">
          <Text className={`text-lg font-semibold text-center ${isLightTheme ? "text-slate-800" : "text-slate-200"}`}>No payroll settings found.</Text>
          <Text className={`mt-2 mb-4 text-center ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>
            Please set up your company’s payroll settings below.
          </Text>
          <Text className={`font-semibold mb-1 mt-4 ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>Cutoff Cycle</Text>
          <DropDownPicker
            open={cutoffOpen}
            value={cutoffValue}
            items={cutoffItems}
          ...
          />
          <Text className={`font-semibold mb-1 mt-4 ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>Currency</Text>
          <DropDownPicker
            open={currencyOpen}
            value={currencyValue}
            items={currencyItems}
            ...
            placeholderStyle={{ color: isLightTheme ? "#6B7280" : "#9CA3AF" }}
          />
          <Text className={`font-semibold mb-1 mt-4 ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>Overtime Rate</Text>
          <View className={`mb-3 p-4 rounded-lg ${isLightTheme ? "bg-slate-100" : "bg-slate-800"}`}>
            <TextInput
              placeholder="e.g. 1.5"
              value={overtimeRate}
          ...
          <Pressable className="bg-orange-500 p-4 rounded-lg mt-6" onPress={handleUpdateSettings}>
            <Text className="text-white text-center font-semibold">Create Payroll Settings</Text>
          </Pressable>
        </View>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView className={`flex-1 ${isLightTheme ? "bg-white" : "bg-slate-900"}`} edges={["top"]}>
      ...
      <View className="flex-1 px-4 mt-4">
        <Text className={`font-semibold mb-1 ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>Cutoff Cycle</Text>
        <DropDownPicker
          open={cutoffOpen}
          value={cutoffValue}
          items={cutoffItems}
