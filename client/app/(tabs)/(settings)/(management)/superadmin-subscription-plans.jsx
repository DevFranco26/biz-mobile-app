...
const SubscriptionPlans = () => {
  ...
  const { subscriptionPlans, loadingPlans, fetchSubscriptionPlans, createSubscriptionPlan, updateSubscriptionPlan, deleteSubscriptionPlan } =
  ...
  const renderPlanItem = ({ item }) => {
    return (
      <View className={`p-4 mb-3 rounded-lg flex-row justify-between ${isLightTheme ? "bg-slate-50" : "bg-slate-800"}`}>
        <Pressable className="flex-row justify-between w-full items-center gap-1" onPress={() => handleViewPlan(item)}>
          <Text className={`text-base font-bold ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>{item.planName}</Text>
          <Text className={`font-medium ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>({item.rangeOfUsers})</Text>
          <Text className={`font-semibold ml-auto ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>${item.price}</Text>
        </Pressable>
      </View>
    );
  };
  ...
  return (
    <SafeAreaView className={`flex-1 ${isLightTheme ? "bg-white" : "bg-slate-900"}`}>
      <View className="px-4 py-3 flex-row items-center">
        ...
        <Text className={`text-lg font-bold ${isLightTheme ? "text-slate-800" : "text-slate-100"}`}>Subscription Plans</Text>
      </View>
      {loadingPlans ? (
          ...
          <View className="flex-1 justify-center items-center bg-black/50">
            <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} className="w-11/12">
              <View className={`p-6 rounded-2xl w-full ${isLightTheme ? "bg-white" : "bg-slate-900"}`}>
                ...
                />
                <Text className={`text-sm mb-1 ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>Description</Text>
                <TextInput
                  className={`w-full p-3 mb-2 rounded-lg border ${
                    isLightTheme ? "border-slate-100 bg-slate-100 text-slate-800" : "border-slate-800 bg-slate-800 text-slate-300"
                  }`}
                ...
                <TextInput
                  ...
                  placeholderTextColor={isLightTheme ? "#9ca3af" : "#6b7280"}
                />
                <Text className={`text-sm mb-1 ${isLightTheme ? "text-slate-800" : "text-slate-300"}`}>Features:</Text>
                <View className="mb-4">
                  <View className="flex-row items-center mb-2">
                    <Text className={`${isLightTheme ? "text-slate-800" : "text-slate-300"} mr-1 text-sm`}>Timekeeping</Text>
                    <Switch
                      value={timekeeping}
                  ...
                  <View className="flex-row items-center mb-2">
                    <Text className={`${isLightTheme ? "text-slate-800" : "text-slate-300"} mr-1 text-sm`}>Payroll</Text>
                    <Switch
                      value={payroll}
      ...
      <Modal visible={viewPlanModalVisible} transparent animationType="fade" onRequestClose={() => setViewPlanModalVisible(false)}>
        <TouchableWithoutFeedback onPress={() => setViewPlanModalVisible(false)}>
          <View className="flex-1 justify-center items-center bg-black/50">
            <View className={`p-6 rounded-2xl w-11/12 ${isLightTheme ? "bg-white" : "bg-slate-800"}`}>
              <Text className={`text-xl font-bold mb-4 ${isLightTheme ? "text-slate-900" : "text-slate-100"}`}>Plan Details</Text>
              {selectedPlan ? (
                <ScrollView>
                  <View className="flex-row items-center mt-2">
                    <Ionicons name="finger-print-outline" size={16} color={isLightTheme ? "#1e293b" : "#cbd5e1"} />
                    <Text className={`ml-2 text-sm ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>Plan ID: {selectedPlan.id}</Text>
                  </View>
                  <View className="flex-row items-center mt-2">
                    <Ionicons name="pricetag-outline" size={16} color={isLightTheme ? "#1e293b" : "#cbd5e1"} />
                    <Text className={`ml-2 text-sm ${isLightTheme ? "text-slate-800" : "text-slate-200"}`}>Subscription: {selectedPlan.planName}</Text>
                  </View>
                  <View className="flex-row items-center mt-2">
                  ...
                  <View className="flex-row items-center mt-2">
                    ...
                    <Text className={`ml-2 text-sm ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>Price: ${selectedPlan.price}</Text>
                  </View>
                  <View className="flex-row items-center mt-2">
                    <Ionicons name="book-outline" size={16} color={isLightTheme ? "#1e293b" : "#cbd5e1"} />
                    <Text className={`ml-2 text-sm ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>Description: {selectedPlan.description}</Text>
                  </View>
                  <Text className={`text-sm mt-4 font-semibold ${isLightTheme ? "text-slate-800" : "text-slate-100"}`}>Features:</Text>
                  {selectedPlan.features && Object.keys(selectedPlan.features).length > 0 ? (
                    ...
                    <Text className={`text-sm ${isLightTheme ? "text-slate-700" : "text-slate-300"}`}>No features listed.</Text>
                  )}
                  <View className="flex-row justify-between mt-8 space-x-2 gap-3">
                    ...
                    <Pressable onPress={() => handleDeletePlan(selectedPlan.id)} className="flex-1 p-4 rounded-lg items-center bg-red-500">
                      <Ionicons name="trash-outline" size={24} color="#fff" />
                    </Pressable>
                    <Pressable onPress={() => setViewPlanModalVisible(false)} className="flex-1 p-4 rounded-lg items-center bg-slate-500">
