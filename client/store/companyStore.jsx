...
const useCompanyStore = create((set, get) => ({
  ...
  fetchCompanies: async (token) => {
    ...
    try {
      // We changed the endpoint from "/api/companies" to "/api/companies/all"
      // only if that is your final route. If your final route is "/api/companies",
      // keep it the same. Adjust accordingly:
  ...
  fetchCompanyById: async (token, companyId) => {
    // Optional: You can skip refetching if already in store:
    const { companies } = get();
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        // Insert or update in the existing companies array
        set((state) => ({
